...
open class Snapshot: NSObject {
    ...
    class func setLaunchArguments(_ app: XCUIApplication) {
        ...
        do {
            ...
            let range = NSRange(location: 0, length: launchArguments.count)
            let matches = regex.matches(in: launchArguments, options: [], range: range)
    ...
    open class func snapshot(_ name: String, timeWaitingForIdle timeout: TimeInterval = 20) {
        ...
        // More information: https://docs.fastlane.tools/actions/snapshot/#how-does-it-work
        NSLog("snapshot: \(name)")
        ...
        if Snapshot.waitForAnimations {
            ...
            // Local fix: XCUIKeyboardKeySecondaryFn was renamed in current SDKs.
            ...
            do {
                // The simulator name contains "Clone X of " inside the screenshot file when
                // running parallelized UI tests on concurrent devices.
    ...
    // Local fix: guarded for macOS (UIImage doesn't exist there). The only
    // call site is already inside `#if os(iOS) && !targetEnvironment(macCatalyst)`.
    #if !os(OSX)
